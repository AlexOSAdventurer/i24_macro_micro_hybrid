...
def run_calibration(trial):
    ...
    v_f = trial.suggest_float("v_f", 45.0, 50.0)
    rho_j = trial.suggest_float("rho_j", 0.12, 0.15)
    lambda_lc = trial.suggest_float("lambda_lc", 0.05, 0.15)
    w = trial.suggest_float("w", 5.0, 8.0)
    """
    {'min_spawn_length': 9.861736157568036, 'min_spawn_distance': 1.4696355112970325, 'spawn_ttc': 1.0790693049454907}
    ...
    spawn_ttc = trial.suggest_float("spawn_ttc", 1.0, 3.0)
    """
    min_spawn_length = trial.suggest_float("min_spawn_length", 5.0, 7.0)
    min_spawn_distance = trial.suggest_float("min_spawn_distance", 2.5, 4.5)
    min_spawn_length += min_spawn_distance
    spawn_ttc = trial.suggest_float("spawn_ttc", 0.45, 0.8)
    ...
    coupler = I24CarlaCoupler(gt, dt=1.0, lanes=[-1, -2, -3, -4], mapping=config, hero_road="2", desired_time=config["time_origin"], desired_s=350.0, visible_window=150.0, ghost_window=0.0, record_videos=False)
    coupler.min_spawn_length = min_spawn_length
    coupler.min_spawn_distance = min_spawn_distance
    coupler.spawn_ttc = spawn_ttc
    ...
    def update_bridge_callback(current_time, resolution):
        ...
        if ((current_time - sim.origin_time) >= (bridge_time_step * (current_bridge_iteration))):
            ...
            coupler = I24CarlaCoupler(gt, dt=1.0, lanes=[-1, -2, -3, -4], mapping=config, hero_road="2", desired_time=sim.current_time, desired_s=350.0, visible_window=150.0, ghost_window=0.0, record_videos=False)
            coupler.min_spawn_length = min_spawn_length
            coupler.min_spawn_distance = min_spawn_distance
            bridge = I24MicroSimBridge(
                sim=sim,
